...
export default defineConfig({
	...
	server: {
		...
		/**
		 * ✅ PROXY DEFINITIVO
		 * - Mantém URL RELATIVA no front: fetch("/api/...")
		 * - Vite encaminha para o BFF em http://localhost:3000
		...
		proxy: {
			"/api": {
				...
				changeOrigin: true,
				secure: false,
				// ajuda em alguns casos com websockets (não atrapalha se não usar)
				ws: true,
				// logs do proxy no terminal do Vite (facilita debug)
				configure(proxy) {
